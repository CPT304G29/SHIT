    ...
    expect(buildRevenueDictionaryRows()).toEqual(
      expect.arrayContaining([
        ...
        { Field: 'Quarterly Period', Meaning: 'YYYY-QN derived from InventoryItem.updatedAt.' },
        {
          ...
          Meaning: 'Daily movement summary generated by the Calendar feature.',
        },
      ])
    );
  });
...
describe('inventory and calendar export sectors', () => {
  ...
  it('marks healthy stock and aggregates duplicate zero-value categories', () => {
    const healthyRows = buildInventoryDetailRows(
      [
        {
          ...items[0],
          id: 'healthy',
    ...
    const categoryRows = buildRevenueCategoryRows(
      [
        {
          ...items[0],
          id: 'zero-1',
          categoryKey: 'category.zero',
          quantity: 0,
          unitPrice: 1000,
        },
        {
    ...
    expect(healthyRows[0]).toMatchObject({
      itemId: 'healthy',
      quantity: 11,
      stockStatus: 'inStock',
    });
